...
const cases = [
  ...
  { name: 'name is only dashes (-------)', expect: 400, field: 'fullName', req: postJson({ ...valid, fullName: '-------' }) },
  { name: 'name is only digits', expect: 400, field: 'fullName', req: postJson({ ...valid, fullName: '12345' }) },
  { name: 'name is only symbols', expect: 400, field: 'fullName', req: postJson({ ...valid, fullName: '!@#$%' }) },
  { name: 'name is only dots', expect: 400, field: 'fullName', req: postJson({ ...valid, fullName: '.....' }) },
  { name: 'name starts with a dash', expect: 400, field: 'fullName', req: postJson({ ...valid, fullName: '-Jane' }) },
  { name: 'name has digits (Jane2)', expect: 400, field: 'fullName', req: postJson({ ...valid, fullName: 'Jane2' }) },
